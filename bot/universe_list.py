...
# The S&P 500, as a dated snapshot generated by scripts/refresh_sp500.py and
# refreshed monthly by a workflow. Imported softly so that a checkout without
# the generated file still loads — the failure then arrives as a clear message
# from get_universe rather than an ImportError from an unrelated module.
try:
    from bot.sp500 import SP500, SP500_SNAPSHOT_DATE
except ImportError:  # pragma: no cover - only before the first refresh
    SP500: tuple[str, ...] = ()
    SP500_SNAPSHOT_DATE = "never"
...
UNIVERSES: dict[str, tuple[str, ...]] = {
    ...
    # Large-cap breadth plus a high-beta tail, deduplicated. Reaches beyond
    # the index, so it holds names with thinner coverage and wider spreads.
...
def get_universe(name: str) -> tuple[str, ...]:
    ...
    symbols = tuple(dict.fromkeys(UNIVERSES[name]))
    if not symbols:
        raise KeyError(
            f"Universe {name!r} is empty. If this is 'sp500', the generated "
            "constituent list is missing: run `python scripts/refresh_sp500.py`, "
            "or the 'Refresh S&P 500' workflow, and commit bot/sp500.py."
        )
    return symbols
